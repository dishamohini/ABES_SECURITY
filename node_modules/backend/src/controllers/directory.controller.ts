 ...
 */
export async function getDirectoryUsers(req: AuthenticatedRequest, res: Response) {
  try {
 ...
 */
export async function createDirectoryUser(req: AuthenticatedRequest, res: Response) {
  const {
    employeeCode,
    name,
    email,
    phone,
    extensionNumber,
    department, // 'Faculty', 'Staff', 'Admin', 'Accounts', 'HR'
    post,
  ...
  try {
    ...
    const passwordHash = await bcrypt.hash('abes123', salt);
    ...
    await createAuditLog(
      ...
      `User:${newUser.id}`,
      `Created directory user: ${name} (${employeeCode}), Department: ${department}`
    );
 ...
 */
export async function deleteDirectoryUser(req: AuthenticatedRequest, res: Response) {
  const { id } = req.params;
  ...
  try {
    ...
    await createAuditLog(
      req.user.id,
      'DELETE_DIRECTORY_USER',
      `User:${id}`,
      `Deleted directory user: ${user.name} (${user.employeeCode})`
    );
 ...
 */
export async function createDirectoryUsersBulk(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    if (importedCount > 0) {
      await createAuditLog(
        req.user.id,
        'BULK_IMPORT_DIRECTORY_USERS',
        'User:Bulk',
        `Successfully imported ${importedCount} users. Skipped ${skippedCount} records.`
      );
    }
 ...
 */
function normalizeRole(roleStr: any, departmentStr: string): string {
  const clean = String(roleStr || '').toLowerCase().trim();
  if (clean.includes('admin')) return 'SECURITY_ADMIN';
  if (clean.includes('guard') || clean.includes('security') || clean.includes('police')) return 'SECURITY_GUARD';
  if (clean.includes('hod') || clean.includes('head') || clean.includes('approver') || clean.includes('director') || clean.includes('dean')) return 'DEPT_HEAD';
  if (clean.includes('faculty') || clean.includes('staff') || clean.includes('teacher') || clean.includes('professor') || clean.includes('employee') || clean.includes('hr')) return 'FACULTY';
  
  // Fallback defaults
  if (departmentStr.toLowerCase() === 'staff') return 'SECURITY_GUARD';
  return 'FACULTY';
}
 ...
 */
export async function deleteDirectoryUsersBulk(req: AuthenticatedRequest, res: Response) {
  const { ids } = req.body;
  ...
  try {
    ...
    await createAuditLog(
      req.user.id,
      'BULK_DELETE_DIRECTORY_USERS',
      'User:Bulk',
      `Bulk deleted ${result.count} users from the directory.`
    );
 ...
 */
export async function updateDirectoryUser(req: AuthenticatedRequest, res: Response) {
  const { id } = req.params;
  const {
    name,
    email,
  ...
  try {
    ...
    await createAuditLog(
      ...
      `User:${id}`,
      `Updated directory user: ${name} (${updatedUser.employeeCode}), Department: ${department}`
    );

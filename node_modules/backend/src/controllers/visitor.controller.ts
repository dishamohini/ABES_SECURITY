...
import { encrypt, decrypt, maskAadhaar, encryptBuffer, decryptBuffer } from '../services/encryption.service';
import { extractAadhaarDetails } from '../services/ocr.service';
...
export type VisitStatus = 'PENDING' | 'APPROVED' | 'REJECTED' | 'COMPLETED';
 ...
 */
export async function runOcr(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const result = await extractAadhaarDetails(buffer);
 ...
 */
export async function registerVisitor(req: AuthenticatedRequest, res: Response) {
  const {
    name,
    phone,
    consentAccepted,
  ...
  try {
    ...
    const photoFilePath = path.join(UPLOAD_DIR, photoFileName);
    fs.writeFileSync(photoFilePath, encryptedPhoto.data);
 ...
 */
export async function getVisitRequest(req: AuthenticatedRequest, res: Response) {
  const { id } = req.params;
 ...
 */
export async function checkInVisitor(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    await createAuditLog(
      ...
      `Logged entry at gate. Request ID: ${requestId}`
    );
 ...
 */
export async function getFacePhoto(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    await createAuditLog(
      req.user?.id || null,
      'VIEW_BIOMETRIC_FACE_PHOTO',
      `Visitor:${visitorId}`,
      `Accessed biometric face image`
    );

    // Read encrypted file
    const encryptedBuffer = fs.readFileSync(photoPath);
 ...
 */
export async function viewAadhaar(req: AuthenticatedRequest, res: Response) {
  const { visitorId } = req.params;
  ...
  try {
    ...
    // Decrypt Aadhaar
    const decryptedAadhaar = decrypt(visitor.aadhaarNumberEncrypted);

    // Audit log the view event (Critical requirement)
    await createAuditLog(
      req.user!.id,
      'VIEW_RAW_AADHAAR_NUMBER',
      `Visitor:${visitorId}`,
      `Viewed decrypted Aadhaar number`
    );
 ...
 */
export async function exitMatchVisitor(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const originalPhotoPath = path.join(UPLOAD_DIR, activeVisit.visitor.facePhotoRef);
    if (!fs.existsSync(originalPhotoPath)) {
      return res.status(404).json({ error: 'Original face biometric not found on server' });
    }

    const encryptedOrigBuffer = fs.readFileSync(originalPhotoPath);
    const [ivHex, authTagHex] = activeVisit.visitor.facePhotoKeyEncrypted.split(':');
    const originalBuffer = decryptBuffer(encryptedOrigBuffer, ivHex, authTagHex);

    // Decode exit photo base64
    const exitBuffer = base64ToBuffer(exitPhotoBase64);

    // 3. Run Face-Match comparison
    const matchResult = await faceRecognitionService.compareFaces(originalBuffer, exitBuffer);

    // Log the face comparison event in audit
    await createAuditLog(
      guardId,
      'VISITOR_EXIT_FACE_MATCH',
      `Visitor:${visitorId}`,
      `Exit face comparison score: ${matchResult.confidence}%, isMatch: ${matchResult.isMatch}`
    );
    ...
    await createAuditLog(
      ...
      'VISITOR_CHECK_OUT',
      `Visitor:${visitorId}`,
      `Checked out visitor successfully with face match (${matchResult.confidence}%)`
    );

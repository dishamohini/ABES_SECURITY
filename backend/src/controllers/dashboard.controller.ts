...
export async function decideApproval(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    await createAuditLog(
      req.user.id,
      `VISITOR_DECISION_${decision}`,
      `Visitor:${request.visitorId}`,
      `Decision: ${decision}. Approver: ${req.user.name}. Note: ${message || ''}`
    );
 ...
 */
export async function guardOverrideApproval(req: AuthenticatedRequest, res: Response) {
  const { id } = req.params;
  ...
  try {
    ...
    await createAuditLog(
      ...
      `Visitor:${request.visitorId}`,
      `Guard override verbal approval. Guard: ${req.user.name}. Note: ${finalMessage}`
    );
 ...
 */
export async function getLiveInside(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const processedVehicles = new Map<string, any>();
    const processedVisitors = new Map<string, any>();
...
export async function getLogs(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    if (direction) {
      whereClause.direction = direction as Direction;
    }
...
export async function getStats(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const usersIn = new Set<string>();
    const vehiclesIn = new Set<string>();
    const visitorsIn = new Set<string>();

    for (const log of allLogs) {
      if (log.userId && !uniqueUsers.has(log.userId)) {
        uniqueUsers.add(log.userId);
        if (log.direction === 'IN') usersIn.add(log.userId);
      }
      if (log.vehicleId && !uniqueVehicles.has(log.vehicleId)) {
        uniqueVehicles.add(log.vehicleId);
        if (log.direction === 'IN') vehiclesIn.add(log.vehicleId);
      }
      if (log.visitorId && !uniqueVisitors.has(log.visitorId)) {
        uniqueVisitors.add(log.visitorId);
        if (log.direction === 'IN') visitorsIn.add(log.visitorId);
      }
    }
